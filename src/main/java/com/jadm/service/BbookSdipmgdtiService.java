...
@Service
@Transactional
public class BbookSdipmgdtiService {
	
	@Autowired
	BbookSdipmgdtiRepository bbooksdipmgdtirepository;
	
	private static final Logger LOG = LoggerFactory.getLogger(BbookSdipmgdtiService.class);

	public List<BbookSdipmgdti>  writeToCsvDetailCsv(String archivo, String ruta, long idn) {		
	       final String pathCompletoArchivoResultanteDescarga = ruta+archivo;
	       final String ColumnasCsv= ";";
	       List<BbookSdipmgdti> ResultsToWriteIntoCsv=null;
...
//	             .withSeparator()
	             .build();
	                ...
	                final String[] fieldNamesToWriteIntoCsv = CsvBbookSdipmgdti.getHeader();
	                ...
	                try {
	                	ResultsToWriteIntoCsv=bbooksdipmgdtirepository.findByDownloadDate1IsNullAndBatchNum(idn);
	               // 	totreg = ResultsToWriteIntoCsv.size();
	                    ...
	                    for(BbookSdipmgdti row : ResultsToWriteIntoCsv) {

	                        final String[] fieldValuesToWriteIntoCsv  = new CsvBbookSdipmgdti(row).getCsvLine();
